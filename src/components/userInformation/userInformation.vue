<template>
  <div>
    ...
    <Form ref="queryForm" :model="queryParams" :labelWidth="68" inline>
      ...
      <Button type="info" size="default" @click="addUser">
        <Icon type="md-add" size="16" />新增
      </Button>
    </Form>
    <!-- 主表格 -->
    <Table highlight-row :columns="columns" :data="data">
  ...
  <template slot-scope="{ row, index }" slot="mobile">
      ...
      <template slot-scope="scope" slot="action">
        <Button type="info" class="ui-margin-right" size="small" @click="editModalRow(scope)">编辑</Button>
        <Button type="success" size="small" @click="removeRow(scope)">删除</Button>
      </template>
    </Table>
    ...
    <template>
     ...
     <!-- 添加弹框 -->
      <Modal v-model="addModal" width="60%" style='fontFamily="微软雅黑"' title="添加用户信息" @on-ok="addmodalConfirm" @on-cancel="addmodalCancel">
        <Form ref="addFormRef" :model="addForm" :label-width="80" :rules="addFormRules">
          ...
          <FormItem label="出生年月"  prop="birthday">
            <DatePicker type="date" placeholder="选择时间" v-model="addForm.birthday" format="yyyy-MM-dd"></DatePicker>
          </FormItem>
        ...
        <div slot="footer">
        <Button type="text" size="large" @click="addmodalCancel">取消</Button>
        <Button type="primary" size="large" @click="addmodalConfirm">确定</Button>
      </div>
      </Modal>
       <!-- 编辑弹框 -->
      <Modal v-model="editModal" width="60%"  title="编辑" @on-ok="editmodalConfirm" @on-cancel="editmodalCancel">
        <Form ref="editFormRef" :model="editForm" :label-width="80" :rules="editFormRules">
          ...
          <FormItem label="出生年月" prop="birthday" >
            <div><DatePicker type="date" placeholder="选择时间"  v-model="editForm.birthday"></DatePicker></div>
         ...
         <div slot="footer">
        <Button type="text" size="large" @click="editmodalCancel">取消</Button>
...
export default {
  ...
  data() {
    return {
      ...
      addFormRules:{
        ...
        birthday:[
           { required: true, message: '请选择出生年月', trigger: 'change',pattern: /.+/ },
          
        ],
        address:[
           { required: true, message: '请选择地址', trigger: 'change',pattern: /.+/  },
        ],
      },
      editFormRules:{
        name:[
  ...
  filters: {
    formateTime: function (value) {
      ...
      value = value.getFullYear() + '-' + ((value.getMonth()+1 )>=10?(value.getMonth()+1):'0'+(value.getMonth()+1)) + '-' + (value.getDate()>10 ? value.getDate():'0'+value.getDate());
      return value
    }
  },
  ...
  methods: {
    ...
    timeFormat(time){
      ...
      const StringTime = formatData.getFullYear() + '-' + (formatData.getMonth() + 1) + '-' + formatData.getDate()
      return StringTime
